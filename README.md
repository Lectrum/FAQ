...
2. Прочитай текст ошибки, попытайся понять её суть. Чаще всего, текст ошибки сам по себе объясняет её причину.
    + Самая частая ошибка: `Error: listen EADDRINUSE 0.0.0.0:3000`. Давай разберёмся как её решить. Читаем:
        ...
        + Итого, можно составить заключение, что адрес `0.0.0.0:3000` чем-то используется. Webpack поднимает проект именно на это адресе. Но так как этот адрес уже занят каким-то процессом — возникает ошибка.
