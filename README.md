...
> То есть, чтобы `Prettier` понимал, что форматировать код нужно именно по правилам `ESLint` — нужно обязательно использовать интеграцию [`prettier-eslint-cli`](https://github.com/prettier/prettier-eslint-cli).

> Для настройки интаграции `Prettier` с `ESlint` в редакторах (чтобы можно было форматировать код по хотекю) — следуй гайдам:
