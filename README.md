...
Если ты по какой-либо причине сильно отстанешь от преподавателя и группы, то догонять может быть непросто.
Для решения этой проблемы мы и ввели возможность синхронизации с преподавателем.
...
сможешь продолжать дальше самостоятельно.
При этом, после выполнения `npm run checkpoint`, **твой код будет сохранён** в отдельной ветке.
...
Перейди в директорию с обучающим проектом, выполни `npm install`, а затем —
`npm run checkpoint`. Программа сообщит тебе детальную информацию о каждом
шаге синхронизации. После окончания синхронизации запусти проект командой
`npm run start` и перейди по адресу `localhost:3000` в браузере. Если ты
видишь сообщение `«Поехали!»`, то синхронизация работает, во время обучения
ты сможешь воспользоваться ей в любой момент. Если ты видишь сообщение
`«Добро пожаловать»!, то на твоём компьютере синхронизация не работает. Но не
...
Не волнуйся. Ты всё равно можешь синхронизироваться с преподавателем, только вручную.
Для этого выполни в консоли следующие команды:
