...
- [🌏 Настройка окружения](#-Настройка-окружения)
    ...
    - [❓ Вопрос: Я пользователь Windows. Как настроить терминал, чтобы работало хорошо?](#-Вопрос-я-пользователь-windows-npm-audit-fix-после-установки-зависимостей)
...
#### ❓ Вопрос: Я пользователь Windows. Как настроить терминал, чтобы работало хорошо?A
✅ Ответ: есть несколько вариантов.
+ `Если у тебя Windows 10`: ты [можешь установить Ubuntu](https://remontka.pro/linux-bash-shell-windows-10/). Это позволит тебе работать на Windows в [UNIX-окружении Ubuntu](https://www.microsoft.com/en-us/p/ubuntu/9nblggh4msv6), что намного лучше для веб-разработчика.
 установил Node.js установщиком с сайта, то обновить тоже нужно с помощью [установщика последней версии](https://nodejs.org/en/).
+ `Если у тебя Windows 8 или более ранняя`: ты можешь установить [`Git Bash`](https://git-scm.com/download/win). Это тоже позволит тебе работать в UNIX-подобном окружении, но не настолько полноценном, как в предыдущем варианте.
