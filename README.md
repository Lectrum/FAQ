...
### Вопрос: что такое [`Prettier`](https://prettier.io/)? 
...
> `ESLint` — это линтер, интрумент для статического анализа кода, который вы пишете. У него тоже есть настройки, исходя от которых он проводит анализ и говорит вам об ошибках
> `Prettier` и `eslint` можно использовать по отдельности, и можно вместе — они друг другу не мешают.
> А `prettier-eslint` — это интеграция первого инструмента со вторым, как мостик между двумя берегами.
> `prettier` 🏖 ← мостик `prettier-eslint` → 🏖 `ESLint`
> То есть чтобы `prettier` понимал, что форматировать нужно по правилам `eslint` — нужно использовать интеграцию в виде `prettier-eslint`
> Для настройки `prettier` с `eslint-интеграцией` в редакторах следуй гайдам: [webStorm](https://prettier.io/docs/en/webstorm.html#using-prettier-with-eslint)
